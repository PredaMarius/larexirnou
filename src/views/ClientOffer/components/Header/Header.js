...
const Header = (props) => {
  return(
    <View style={styles.section}>
      ...
      <Text style={styles.titlu}>{`OFERTA ${props.currentProduct.title.toUpperCase()}`}</Text>
      

      <Text style={styles.dateComanda}>{`Numar: ${props.currentOrder.id}     Data: ${dataFormatRO(props.currentOrder.dataComanda)}`}</Text>
      <Text style={styles.dateComanda}>{`Client: ${props.currentOrder.clientClient}`}</Text>
